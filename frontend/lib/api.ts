...
import { connectionStatusActions } from "@/contexts/ConnectionContext";
import {
  ServerMessage,
  MessageServer,
  MessageChannel,
...
const API_PREFIX = "https://crossmind.reponchain.com/api";
...
const fetcher = async ({
  ...
  // Handle URL construction correctly with full URL prefix
  const normalizedUrl = API_PREFIX + (url.startsWith("/") ? url : `/${url}`);
  clientLogger.info("Normalized URL:", normalizedUrl);
...
export const apiClient = {
  ...
  createCentralGroupChat: (payload: {
    ...
    fetcher({
      url: "/messaging/central-channels",
      method: "POST",
      body: payload,
    }),
  ...
  transcribeAudio: async (
    ...
    return fetcher({
      url: `/audio/${agentId}/transcriptions`,
      method: "POST",
      body: formData,
    });
  },
  ...
  clearGroupChat: (serverId: UUID): Promise<void> =>
    fetcher({
      url: `/messaging/central-channels/${serverId}/messages`,
      method: "DELETE",
    }),
  ...
  addAgentToServer: (
    ...
    fetcher({
      url: `/messaging/servers/${serverId}/agents`,
      method: "POST",
      body: { agentId },
    }),
  ...
  removeAgentFromServer: (
    ...
    fetcher({
      url: `/messaging/servers/${serverId}/agents/${agentId}`,
      method: "DELETE",
    }),
